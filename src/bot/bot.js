...
bot.command("privacy", async (ctx) => {
 ...
 bot.on("text", async (ctx) => {
  try {
    ...
    // 🔒 Глобальная проверка на кириллицу
    if (/[\u0400-\u04FF]/.test(userInput)) {
      return ctx.reply("Пожалуйста, вводите данные латиницей, как в документах. Кириллица не допускается.");
    }
    ...
    await ctx.reply("❌ Ошибка при обработке ответа. Попробуйте ещё раз.");
  }
});
